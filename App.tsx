...
const App: React.FC = () => {
  ...
  // Load initial state with AppaFlow specific keys
  useEffect(() => {
  ...
  // Persist state
  useEffect(() => {
    if (transactions.length > 0) localStorage.setItem('appaflow_txs', JSON.stringify(transactions));
  }, [transactions]);
  ...
  const handleTransactionsFound = (newTxs: Transaction[]) => {
  ...
  const handleOnboardingComplete = () => {
    ...
    localStorage.setItem('appaflow_onboarded', 'true');
  };

  const clearData = () => {
      if(confirm('Clear all data from AppaFlow?')) {
          setTransactions([]);
          setRules([]);
  ...
  const exportPDF = () => {
      alert("AppaFlow report generation started...");
  }
  ...
  const getBackgroundStyle = (currentView: AppView) => {
    switch (currentView) {
      case AppView.TRANSACTIONS:
        return {
          backgroundImage: `radial-gradient(at 50% -20%, rgba(124, 58, 237, 0.15) 0px, transparent 50%), radial-gradient(at 100% 40%, rgba(99, 102, 241, 0.1) 0px, transparent 50%), radial-gradient(at 0% 100%, rgba(56, 189, 248, 0.15) 0px, transparent 50%)`
        };
      case AppView.SETTINGS:
        return {
          backgroundImage: `radial-gradient(at 90% 10%, rgba(245, 158, 11, 0.1) 0px, transparent 50%), radial-gradient(at 10% 90%, rgba(244, 63, 94, 0.15) 0px, transparent 50%), radial-gradient(at 50% 50%, rgba(99, 102, 241, 0.05) 0px, transparent 50%)`
        };
      case AppView.DASHBOARD:
      default:
        return {
          backgroundImage: `radial-gradient(at 0% 0%, rgba(99, 102, 241, 0.15) 0px, transparent 50%), radial-gradient(at 100% 0%, rgba(16, 185, 129, 0.1) 0px, transparent 50%), radial-gradient(at 100% 100%, rgba(244, 63, 94, 0.1) 0px, transparent 50%), radial-gradient(at 0% 100%, rgba(56, 189, 248, 0.1) 0px, transparent 50%)`
        };
    }
  };
    ...
    >
      ...
      <header className="px-5 py-4 flex items-center justify-between bg-slate-950/40 backdrop-blur-md sticky top-0 z-20 border-b border-white/5">
        <div className="flex items-center gap-3">
             <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-primary-500/20 ring-1 ring-white/10 shrink-0">
        ...
        <div className="flex items-center gap-2">
            {transactions.length > 0 && (
                 <button onClick={exportPDF} className="text-[10px] font-bold uppercase tracking-wider bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition-colors border border-white/10">
                     Export
                 </button>
            )}
      ...
      <main className="flex-1 overflow-hidden relative w-full mx-auto">
        <div className="h-full overflow-y-auto scrollbar-hide">
            ...
            {view === AppView.TRANSACTIONS && <TransactionList transactions={transactions} categories={categories} />}
            {view === AppView.SETTINGS && (
              <Settings 
                rules={rules} 
                onUpdateRules={setRules} 
                onApplyRules={handleApplyRulesRetroactively}
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center py-3 pt-4 transition-all duration-200 relative group ${isActive ? 'text-primary-400' : 'text-slate-500 hover:text-slate-300'}`}
            >
              {isActive && (
                <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-primary-500 to-transparent shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
              )}
